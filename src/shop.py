...
class Shop:
    ...
    def handle_click(self, pos, player):
        ...
        # Check buy button
        buy_button_rect = pygame.Rect(self.width - 120, self.height - 50, 80, 30)
        if buy_button_rect.collidepoint(rel_x, rel_y) and self.selected_item:
    ...
    def render(self, screen):
        ...
        title_text = font.render(self.name, True, (255, 255, 255))
        title_rect = title_text.get_rect(center=(self.width // 2, 30))
        shop_surface.blit(title_text, title_rect)
        ...
        for i, item in enumerate(self.items):
            ...
            pygame.draw.rect(shop_surface, slot_color, slot_rect)
            pygame.draw.rect(shop_surface, (200, 200, 200), slot_rect, 2)
            ...
            if hasattr(item, 'sprite') and item.sprite:
                ...
                shop_surface.blit(scaled_sprite, sprite_rect)
            else:
                # Fallback colored rectangle
                item_colors = {
                    "weapon": (192, 192, 192),
                    "armor": (165, 42, 42),
                    "consumable": (255, 0, 0) if "Health" in item.name else (0, 0, 255)
                }
                ...
                pygame.draw.rect(shop_surface, color, inner_rect)
        ...
        if self.selected_item:
            details_y = items_start_y + ((len(self.items) - 1) // self.slots_per_row + 1) * (self.slot_size + self.padding) + 20
            
            # Item name
            name_font = pygame.font.Font(None, 24)
            name_text = name_font.render(self.selected_item.name, True, (255, 255, 255))
            shop_surface.blit(name_text, (self.padding, details_y))
            
            # Item effects
            effects_y = details_y + 25
            ...
            for effect_name, effect_value in self.selected_item.effect.items():
                ...
                shop_surface.blit(effect_surface, (self.padding, effects_y))
                effects_y += 20
            
            # Price
            price_text = f"Price: {self.selected_item.value} gold"
            price_surface = effect_font.render(price_text, True, (255, 215, 0))
            shop_surface.blit(price_surface, (self.padding, effects_y))
            
            # Buy button
            buy_button_rect = pygame.Rect(self.width - 120, self.height - 50, 80, 30)
            button_color = (50, 150, 50) if True else (100, 100, 100)  # Always enabled for now
            pygame.draw.rect(shop_surface, button_color, buy_button_rect)
            pygame.draw.rect(shop_surface, (255, 255, 255), buy_button_rect, 2)
            ...
            shop_surface.blit(buy_text, buy_text_rect)
        
        # Instructions
        instruction_font = pygame.font.Font(None, 18)
        instructions = [
            "Click items to select, then click Buy to purchase",
            "Click X or outside window to close shop"
        ]
        
        for i, instruction in enumerate(instructions):
            instruction_surface = instruction_font.render(instruction, True, (180, 180, 180))
            shop_surface.blit(instruction_surface, (self.padding, self.height - 40 + i * 15))
